 ...
 */
void SocketHelper::createSocket(int *sock_fd, bool send) {
  // creating socket
  *sock_fd = socket(AF_INET, SOCK_DGRAM, 0);

  struct timeval timeout;
  timeout.tv_sec = (uint)(TIMEOUT / 1000);
  ...
  if (send)
    bind_feedback =
        bind(*sock_fd, (struct sockaddr *)&serv_addr, sizeof(sockaddr));
  else
    bind_feedback =
        bind(*sock_fd, (struct sockaddr *)dstIpAddr, sizeof(sockaddr));
...
/// helper function that send a packet over the socket.
// it respects timeouts
void SocketHelper::sendPacket(int sock_fd, char *buffer, ssize_t len) {
  ssize_t n;
  int timeoutCounter = 0;
  char tmp[sizeof(Packet)];

  while (true) {
    n = sendto(sock_fd, buffer, len, 0, (struct sockaddr *)dstIpAddr,
               sizeof(sockaddr));
    ...
    ssize_t recv_size = recv(sock_fd, tmp, sizeof(Packet), 0);

    if (recv_size < 0) {
      perror("recv");
      std::cout << "timeout at packet. Sending Packet "
                << reinterpret_cast<PacketHeader *>(buffer)->sequenceNumber
                << " again" << std::endl;
    } else {
      std::cout << "recv ack: " << recv_size << std::endl;
      // std::cout << *reinterpret_cast<Packet *>(tmp) << std::endl;
      break;
    }
  }
      ...
      std::visit(
          [&sock_fd, this](auto &&arg) {
            ...
            if constexpr (std::is_same_v<T, Packet *>) {
              ...
              for (int i = 0; i < reinterpret_cast<Packet *>(arg)->dataLen;
                   i++) {
                packetAsArray[i + sizeof(PacketHeader)] =
                    reinterpret_cast<Packet *>(arg)->data[i];
              }

              sendPacket(sock_fd, packetAsArray,
                         sizeof(PacketHeader) +
                             reinterpret_cast<Packet *>(arg)->dataLen);
              ...
              memcpy(packetAsArray, arg,
                     sizeof(PacketHeader) + sizeof(uint32_t));
              for (int i = 0; i < reinterpret_cast<StartPacket *>(arg)->nameLen;
                   i++) {
              ...
              sendPacket(sock_fd, packetAsArray,
                         sizeof(uint32_t) + sizeof(PacketHeader) +
                             reinterpret_cast<StartPacket *>(arg)->nameLen);
              ...
              sendPacket(sock_fd, packetAsArray, sizeof(EndPacket));
...
void SocketHelper::runSlave(const bool *run) {
  ...
  while (*run) {
    ...
    sendto(sock_fd, "ACK", 3, 0, (struct sockaddr *)&lastCon, sizeof(sockaddr));
