...
public class Transmitter {
    ...
    private static final int MAX_PACKET_SIZE = 16000; // 9 KB max. Übertragungsgröße pro paket
    ...
    private static void sendFile(DatagramSocket socket) throws IOException, NoSuchAlgorithmException {
        ...
        stopAndWait(socket, packet, sequenceNumber);
        ...
        stopAndWait(socket, eofPacket, sequenceNumber);
    ...
    public static void stopAndWait(DatagramSocket socket, DatagramPacket packet, int sequenceNumber) throws IOException {
        boolean acknowledged = false;
        while (!acknowledged) {
            socket.send(packet);
            try {
                socket.setSoTimeout(TIMEOUT);
                socket.receive(packet);  // Bestätigung abwarten (1 sek. --> das ist das TIMEOUT
                acknowledged = true;
